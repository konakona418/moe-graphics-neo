...
namespace {
    // Black-and-white halftone (print) demo reusing the toon scene:
    //   pass 1: scene (halftone / halftone_spec per material) -> scene target
    ...
    struct HalftoneData {
        ...
        moe::neo::ProgramHandle mHalftoneProgram;
        moe::neo::ProgramHandle mHalftoneSpecProgram;
        ...
        float mHalftoneScale{16.0f};
        float mHalftoneAngleDeg{45.0f};
        float mShadowEdge{0.80f};
        float mShadowSoftness{0.08f};
        float mLineWidth{2.0f};
        float mInkStrength{0.92f};
    ...
    moe::neo::Texture MakeNoiseAtlas(uint32_t tileSize, uint32_t gridSize) {
        ...
        texture.mName = "halftone_noise_atlas";
    ...
    void ApplyMaterialParams(HalftoneData& data) {
        const float angle = glm::radians(data.mHalftoneAngleDeg);
        for (const char* name : kMaterialNames) {
            data.mModel.SetMaterialParam(name, "halftoneScale", data.mHalftoneScale);
            data.mModel.SetMaterialParam(name, "halftoneAngle", angle);
            data.mModel.SetMaterialParam(name, "shadowEdge", data.mShadowEdge);
            data.mModel.SetMaterialParam(name, "shadowSoftness", data.mShadowSoftness);
            data.mModel.SetMaterialParam(name, "lineWidth", data.mLineWidth);
            data.mModel.SetMaterialParam(name, "inkStrength", data.mInkStrength);
            data.mModel.SetMaterialParam(name, "specStrength", data.mSpecStrength);
            data.mModel.SetMaterialParam(name, "screenSpace",
                    data.mScreenSpace ? 1.0f : 0.0f);
            data.mModel.SetMaterialParam(name, "screenCellPixels", data.mScreenCellPixels);
        }
    }

    bool Setup(void* userdata, examples::AppContext& ctx) {
        auto* data = static_cast<HalftoneData*>(userdata);

        // procedural scene: one mesh + material + node per shape
        moe::neo::Scene scene;
        scene.mName = "halftone";
        ...
        // grayscale paper colors: lit surfaces are flat, shadows get the
        // halftone screen
        scene.mMaterials.push_back(MakeMaterial("sphere", {0.75f, 0.75f, 0.75f, 1.0f}));
        scene.mMaterials.push_back(MakeMaterial("torus", {0.70f, 0.70f, 0.70f, 1.0f}));
        scene.mMaterials.push_back(MakeMaterial("box", {0.85f, 0.85f, 0.85f, 1.0f}));
        scene.mMaterials.push_back(MakeMaterial("ground", {0.90f, 0.90f, 0.90f, 1.0f}));
        ...
        if (!data->mModel.IsValid()) {
            std::fprintf(stderr, "halftone: scene upload: %s\n", moe::Error::Get().c_str());
            return false;
        }
        ...
        data->mHalftoneSpecProgram = ctx.mAssets.LoadGraphicsProgram(
                (std::string(shaderDir) + "halftone_spec.vert.spv").c_str(),
                (std::string(shaderDir) + "halftone_spec.frag.spv").c_str());
        data->mNormalProgram = ctx.mAssets.LoadGraphicsProgram(
                (std::string(shaderDir) + "halftone_normal.vert.spv").c_str(),
                (std::string(shaderDir) + "halftone_normal.frag.spv").c_str());
        data->mCompositeProgram = ctx.mAssets.LoadGraphicsProgram(
                (std::string(shaderDir) + "halftone_composite.vert.spv").c_str(),
                (std::string(shaderDir) + "halftone_composite.frag.spv").c_str());
        if (!data->mHalftoneProgram.IsValid() || !data->mHalftoneSpecProgram.IsValid()
                || !data->mNormalProgram.IsValid() || !data->mCompositeProgram.IsValid()) {
            std::fprintf(stderr, "halftone: shader load: %s\n", moe::Error::Get().c_str());
            return false;
        }

        data->mNoiseAtlas = ctx.mAssets.UploadTexture(MakeNoiseAtlas(256, 8));
        if (!data->mNoiseAtlas.IsValid()) {
            std::fprintf(stderr, "halftone: noise atlas: %s\n", moe::Error::Get().c_str());
            return false;
        }
        // per-material program: the torus gets the specular variant
        data->mModel.SetMaterialProgram("torus", data->mHalftoneSpecProgram);
        ...
        if (!data->mRenderer.Init(ctx.mDevice, ctx.mPipelineCache,
            ...
            std::fprintf(stderr, "halftone: renderer: %s\n", moe::Error::Get().c_str());
            return false;
        }
        ...
        if (!data->mSceneTarget.IsValid() || !data->mNormalTarget.IsValid()) {
            std::fprintf(stderr, "halftone: render targets: %s\n", moe::Error::Get().c_str());
            return false;
        }
        ...
        if (data->mCompositeResolution < 0 || data->mCompositeAmplitude < 0
            ...
            std::fprintf(stderr, "halftone: composite push constant names mismatch\n");
            return false;
        }
        return true;
    }
    ...
    void PostRender(void* userdata, examples::AppContext& ctx, moe::rhi::CommandList& cmd) {
        ...
        const moe::neo::PassDesc compositePass{"halftone ink", {}, {}};
    ...
    void DrawUI(void* userdata, examples::AppContext&) {
        auto* data = static_cast<HalftoneData*>(userdata);
        ImGui::Begin("halftone demo");
        ImGui::Text("FPS: %.1f", ImGui::GetIO().Framerate);
        bool changed = false;
        changed |= ImGui::SliderFloat("screen scale", &data->mHalftoneScale, 4.0f, 48.0f);
        changed |= ImGui::SliderFloat("screen angle", &data->mHalftoneAngleDeg, 0.0f, 90.0f);
        changed |= ImGui::Checkbox("screen space", &data->mScreenSpace);
        changed |= ImGui::SliderFloat("screen cell (px)", &data->mScreenCellPixels, 4.0f, 32.0f);
        changed |= ImGui::SliderFloat("shadow edge", &data->mShadowEdge, 0.0f, 1.0f);
        changed |= ImGui::SliderFloat("shadow softness", &data->mShadowSoftness, 0.0f, 0.5f);
        changed |= ImGui::SliderFloat("line width (px)", &data->mLineWidth, 0.5f, 5.0f);
        changed |= ImGui::SliderFloat("ink strength", &data->mInkStrength, 0.0f, 1.0f);
        ...
        ImGui::ColorEdit4("outline color", data->mInkColor);
        ImGui::SliderFloat("outline width (px)", &data->mInkWidth, 0.5f, 6.0f);
        ImGui::SliderFloat("outline jitter (px)", &data->mJitterAmplitude, 0.0f, 4.0f);
        ImGui::SliderFloat("outline noise scale", &data->mNoiseScale, 1.0f, 12.0f);
        ImGui::SliderFloat("outline rate (Hz)", &data->mInkRate, 0.0f, 24.0f);
...
int main() {
    ...
    examples::App app;
    if (!app.Run("halftone demo", 1280, 720, callbacks)) {
        std::fprintf(stderr, "halftone: app: %s\n", moe::Error::Get().c_str());
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
}
